...
public class Game {
    ...
    private final ArrayList<Entry<Integer, Integer>> blackSpaces; //explain this
    ...
    public Entry<Integer, Integer> setCapPiece (Entry<Integer, Integer> oldSpace, Entry<Integer, Integer> newSpace){
        ...
        if (Objects.equals(oldSpace.getKey(), newSpace.getKey())) {
        ...
        if (newSpace.getKey() == oldSpace.getKey()+2) {
            ...
            if (newSpace.getValue() == oldSpace.getValue()+2 &&
                    board.getSpace(new Entry<>(oldSpace.getKey()+1, oldSpace.getValue()+1)) != curTurn) {
                capPiece = new Entry<>(oldSpace.getKey()+1, oldSpace.getValue()+1);
            }
            //checks if the move is moving down 2 columns
            else if (newSpace.getValue() == oldSpace.getValue()-2 &&
                    board.getSpace(new Entry<>(oldSpace.getKey()+1, oldSpace.getValue()-1)) != curTurn) {
                capPiece = new Entry<>(oldSpace.getKey()+1, oldSpace.getValue()-1);
            }
        }
    ...
    public boolean isDiag1(Entry<Integer, Integer> oldSpace, Entry<Integer, Integer> newSpace) {
        ...
        if (newSpace.getKey() == oldSpace.getKey()+1 || newSpace.getKey() == oldSpace.getKey()-1) {
            ...
            if (newSpace.getValue() == oldSpace.getValue()+1 ||
                    newSpace.getValue() == oldSpace.getValue() ||
                    newSpace.getValue() == oldSpace.getValue()-1) {
                return true;
            }
        }
        return false;
    ...
    public boolean isValidMove(boolean noo, Entry<Integer, Integer> entry) {
        ...
        if (!noo && board.getSpace(entry) != curTurn) {
